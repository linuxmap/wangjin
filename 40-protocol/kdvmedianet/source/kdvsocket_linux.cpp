...
u16 KdvSocketStartup()
{
    if(0 == g_nRefCount)
    {
        ...
        SOCKHANDLE *pSockHandle = (SOCKHANDLE *)malloc(g_nMaxRcvSockNum * sizeof(SOCKHANDLE));
        if(NULL == pSockHandle)
        {
            return ERROR_SND_MEMORY;
        }
        memset(pSockHandle, 0, g_nMaxRcvSockNum * sizeof(SOCKHANDLE));
        g_tRcvSockList.m_pSockHd = pSockHandle;
...
u16 KdvSocketCleanup()
{
    ...
    if(nExit==1)
    {
        ...
        g_tRcvSockList.m_nSockCount = 0;
        if(g_tRcvSockList.m_pSockHd)
        {
            free(g_tRcvSockList.m_pSockHd);
            g_tRcvSockList.m_pSockHd = NULL;
        }
...
void* SocketRcvTaskProc(void * pParam)
{
    ...
    u8  *pBuf = NULL;
    MEDIANET_MALLOC(pBuf, dwBufLen);
    if(pBuf == NULL)
    {
        OspPrintf(TRUE, FALSE, "[medianet]SocketRcvTaskProc new pBuf fail\n");
        return 0;
    }
    s32     nRcvNum  = 0;
    FRAMEHDR tFrmHdr;
    memset(&tFrmHdr, 0, sizeof(tFrmHdr));
    s32 nEpollEventCount = 0;
    int nEpollFd = epoll_create(MEDIA_SETSIZE);
    if (-1 == nEpollFd)
    {
        OspPrintf(TRUE, FALSE, "epoll_create error!\n ");
        MEDIANET_SAFE_FREE(pBuf);
        return 0;
    }
    epoll_event* ptEpollEvent = new epoll_event[MEDIA_SETSIZE];
    if(ptEpollEvent == NULL)
    {
        OspPrintf(TRUE, FALSE, "[medianetrcvtask] alloc for epollevent fail\n");
        MEDIANET_SAFE_FREE(pBuf);
        return 0;
    }
    memset( ptEpollEvent, 0, sizeof(epoll_event)*MEDIA_SETSIZE);

    epoll_event tEpollEvent;
    memset(&tEpollEvent, 0, sizeof(tEpollEvent));
    u64 qwEventData64 = 0;

    u16 wSockCnt = 0;
    tEpollEvent.events = EPOLLIN | EPOLLPRI;
    tEpollEvent.data.u64 =  (u32)g_hWatchSocket;
    if( 0 != epoll_ctl( nEpollFd , EPOLL_CTL_ADD , g_hWatchSocket , &tEpollEvent ) )
    {
        printf("epoll_ctl add connecting socket failed! errno(%d)(%s)", errno, strerror(errno));
        MEDIANET_SAFE_FREE(pBuf);
        SAFE_DELETE_ARRAY(ptEpollEvent);
        return 0;
    }
    s32 nEpollIndex = 0;
    CKdvSocket *pcKdvSocket = NULL;
    SOCKHANDLE    hSockHandle = INVALID_SOCKET;
    epoll_event    *ptEventTemp = NULL;
    ...
    while (TRUE)
    {
        ...
        if(5 == g_nShowDebugInfo || 255 == g_nShowDebugInfo)
        {
            OspPrintf(TRUE, FALSE, "[SocketRcvTaskProc] select is running   \n");
        }
        nEpollEventCount = epoll_wait(nEpollFd, ptEpollEvent, MEDIA_SETSIZE, -1);
        if(nEpollEventCount <= 0)
        {
            //出错
            if(nEpollEventCount < 0)
            {
                OspPrintf(TRUE, FALSE, "epoll wait fail, errorno(%d)(%s)\n", errno, strerror(errno));
            }
            continue;
        }
        for(nEpollIndex=0; nEpollIndex<nEpollEventCount; nEpollIndex++)
        {
            ptEventTemp = ptEpollEvent + nEpollIndex;
            if( 0 == (ptEventTemp->events & (EPOLLIN | EPOLLPRI | EPOLLERR)))
            {
                continue;
            }

            hSockHandle = (SOCKHANDLE)((u32)ptEventTemp->data.u64);
            BOOL32 bFindSock = FALSE;
            for(wSockCnt = 0; wSockCnt < g_tRcvSockList.m_nSockCount; wSockCnt++)
            {
                if(hSockHandle == g_tRcvSockList.m_pSockHd[wSockCnt])
                {
                    pcKdvSocket = g_tRcvSockList.m_pptSockUnit[wSockCnt];
                    bFindSock = TRUE;
                    break;
                }
                else
            ...
            if(!bFindSock && hSockHandle != g_hWatchSocket)
            {
                if(255 == g_nShowDebugInfo)
                {
                    OspPrintf(1, 0, "[SocketRcvTaskProc] Can not find sock \n");
                }
                continue;
            }


            if(hSockHandle == INVALID_SOCKET)
            {
            continue;
        }
            //接收控制消息
            if(hSockHandle == g_hWatchSocket)
        {
            if( 25 == g_nShowDebugInfo || 255 == g_nShowDebugInfo)
            {
                ...
                u32 dwIndex = tMsgData.m_dwIndex;
                    ...
                    if( 25 == g_nShowDebugInfo || 255 == g_nShowDebugInfo)
                    {
                        OspPrintf(TRUE, FALSE, "[createsocket] recv msg mode:[%d], pmain:[%x]=u32[%lld], index:[%d]\n",
                            dwMode, pMain, tMsgData.m_lluContext, dwIndex);
                    }
                    ...
                    case SOCK_CREATE:   //创建socket
                        {
                            if(g_tRcvSockList.m_nSockCount >= g_nMaxRcvSockNum)
                            {
                                ...
                                if( NULL == pSockHandle)
                                {
                                    OspPrintf(1 ,0, "[SocketRcvTaskProc] exit for no more memory \n");
                                    return 0;
                                }
                                memset(pSockHandle, 0, sizeof(g_nMaxRcvSockNum * sizeof(SOCKHANDLE)));
                                memcpy(pSockHandle, g_tRcvSockList.m_pSockHd, g_tRcvSockList.m_nSockCount*sizeof(CKdvSocket*));
                                free(g_tRcvSockList.m_pSockHd);
                                g_tRcvSockList.m_pSockHd = pSockHandle;
                            ...
                            {
                                if(g_tRcvSockList.m_pptSockUnit[nPos] == pMain && g_tRcvSockList.m_pSockHd[nPos] == pMain->m_hSocket) //complete match.
                                {
                                    bFind=TRUE;
                                    break;
                                }
                            ...
                            if(!bFind)
                            {
                                ...
                                g_tRcvSockList.m_pSockHd[g_tRcvSockList.m_nSockCount-1] = pMain->m_hSocket;
                            }

                            memset( &tEpollEvent , 0 , sizeof(epoll_event) );
                            tEpollEvent.events = EPOLLIN | EPOLLPRI;
                            tEpollEvent.data.u64 = (u64)pMain->m_hSocket;
                            if( 0 != epoll_ctl( nEpollFd , EPOLL_CTL_ADD , pMain->m_hSocket , &tEpollEvent))
                            {
                                printf("epoll_ctl add connecting socket failed! errno(%d)(%s)", errno, strerror(errno));
                            }
                    ...
                    case SOCK_DEL:
                        {
                            ...
                            {
                                if(g_tRcvSockList.m_pptSockUnit[i] == pMain && g_tRcvSockList.m_pSockHd[i] == pMain->m_hSocket) //complete match.
                                {
                                    continue;
                                }
                                g_tRcvSockList.m_pptSockUnit[nTemp] = g_tRcvSockList.m_pptSockUnit[i];
                                g_tRcvSockList.m_pSockHd[nTemp] = g_tRcvSockList.m_pSockHd[i];
                                if (nTemp != i)
                                {
                                    g_tRcvSockList.m_pptSockUnit[i] = NULL;
                                    g_tRcvSockList.m_pSockHd[i] = NULL;
                                }
                                nTemp++;
                            }
                            ...
                            tEpollEvent.events = EPOLLIN | EPOLLPRI;
                            tEpollEvent.data.u64 = (u64)pMain->m_hSocket;
                            if(0 != epoll_ctl(nEpollFd, EPOLL_CTL_DEL, pMain->m_hSocket, &tEpollEvent))
                            {
                                printf("epoll_ctl del connecting socket failed! errno(%d)(%s)", errno, strerror(errno));
                            }
            ...
            else//接收码流
            {
                do
                {
                    nRcvNum = recvfrom(    hSockHandle, (s8 *)pBuf, dwBufLen, 0, NULL, NULL);
                    if(nRcvNum <= 0)
                    {
                        if(5 == g_nShowDebugInfo)
                        {
                            OspPrintf(TRUE,FALSE, "[SocketRcvTaskProc] recvfrom Exception: nRcvNum = %d\n", nRcvNum);
                        }
                        break;
                    }
                    u32 dwTick1 = OspTickGet();
                    pcKdvSocket->CallBack(pBuf, nRcvNum);
    ...
    OspPrintf(TRUE, FALSE, "[SocketRcvTaskProc]  is running over.....  \n");
